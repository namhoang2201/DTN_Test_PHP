...
if (($billId != FALSE) && ($billName != FALSE) && ($amount != FALSE) && ($catId != FALSE)) {
    ...
    if (mysqli_connect_errno()) {
        ...
        if (!$search || mysqli_num_rows($search) == 0) {
            ...
            echo '<tr align="right">';
            echo "<td align='center' width='5%'><input class='checkbox' type='checkbox' id='" . $billId . "' name='list[" . $billId . "]'/></td>";
            echo "<td width='10%'>". $billId ."</td>";
            if ($isPaid == 0) {
                echo "<td width='20%'>" . $billName . "</td>";
            } else {
                echo "<td width='20%' style='text-decoration: line-through;'>" . $billName . "</td>";
            }
            echo "<td width='15%'> $ " . $amount . "</td>";
            echo "<td width='15%'>" . $catId . "</td>";

            if ($isPaid == 0) {
                echo "<td width='15%'><input type='checkbox'></td>";
            } else {
                echo "<td width='15%'><input type='checkbox' checked></td>";
            }
